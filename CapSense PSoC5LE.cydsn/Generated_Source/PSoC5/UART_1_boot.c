...
static uint8  UART_1_started = 0u;
...
void UART_1_CyBtldrCommStart(void) 
{
    ...
    UART_1_Start(0u, UART_1_DWR_POWER_OPERATION);
...
{
    UART_1_EnableOutEP(UART_1_BTLDR_OUT_EP); 
}
                                                            ...
                                                            
{
    ...
    UART_1_LoadInEP(UART_1_BTLDR_IN_EP, pData, UART_1_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait unitl host reads data from IN endpoint. */
    while ((UART_1_GetEPState(UART_1_BTLDR_IN_EP) == UART_1_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(UART_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (UART_1_GetEPState(UART_1_BTLDR_IN_EP) == UART_1_IN_BUFFER_FULL)
    {
                                                            ...
                                                            
{
    ...
    if (size > UART_1_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = UART_1_BTLDR_SIZEOF_WRITE_BUFFER;
    }

    /* Wait for enumeration first time. */
    if (0u != UART_1_started)
    {
        /* Wait for device enumeration. */
        while ((0u == UART_1_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(UART_1_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        if (0u != UART_1_GetConfiguration())
        {
            (void) UART_1_IsConfigurationChanged();  /* Clear configuration changes state status. */
            UART_1_CyBtldrCommReset();
    ...
    {
        if (0u != UART_1_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET. */
        {
            if (0u != UART_1_GetConfiguration())   /* Init OUT endpoints when device reconfigured. */
            {
                UART_1_CyBtldrCommReset();
            }
        }
    ...
    while ((UART_1_GetEPState(UART_1_BTLDR_OUT_EP) != UART_1_OUT_BUFFER_FULL) && \
           (0u != timeoutMs))
    {
        CyDelay(UART_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (UART_1_GetEPState(UART_1_BTLDR_OUT_EP) == UART_1_OUT_BUFFER_FULL)
    {
        *count = UART_1_ReadOutEP(UART_1_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
