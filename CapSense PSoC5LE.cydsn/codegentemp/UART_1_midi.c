...
***************************************/
    ...
    static const uint8 CYCODE UART_1_MIDI_SIZE[] = {
...
***************************************/
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 UART_1_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 UART_1_midiInPointer;                             /* Input endpoint buffer pointer */
    ...
    volatile uint8 UART_1_midi_in_ep;
    ...
    uint8 UART_1_midiInBuffer[UART_1_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
    ...
    volatile uint8 UART_1_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 UART_1_midiOutBuffer[UART_1_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (UART_1_MIDI_OUT_BUFF_SIZE > 0) */

#if (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF)

    static UART_1_MIDI_RX_STATUS UART_1_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 UART_1_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 UART_1_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF)
        static UART_1_MIDI_RX_STATUS UART_1_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 UART_1_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref UART_1_MIDI1_InqFlags*/
        volatile uint8 UART_1_MIDI2_InqFlags;                             /* Device inquiry flag */
...
#if (UART_1_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: UART_1_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref UART_1_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. UART_1_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref UART_1_midiOutBuffer: This buffer is used by the
*    UART_1_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref UART_1_callbackLocalMidiEvent function.
*
*  \ref UART_1_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref UART_1_midi_in_ep: Used as an IN endpoint number.
*
*   \ref UART_1_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void UART_1_MIDI_Init(void) 
{
   ...
   UART_1_midiInPointer = 0u;
        ...
        UART_1_LoadInEP(UART_1_midi_in_ep, UART_1_midiInBuffer,
                                                               UART_1_MIDI_IN_BUFF_SIZE);
    #endif  /* (UART_1_MIDI_IN_BUFF_SIZE > 0) */

    #if (UART_1_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)UART_1_ReadOutEP(UART_1_midi_out_ep, UART_1_midiOutBuffer,
                                                                       UART_1_MIDI_OUT_BUFF_SIZE);
    #endif /* (UART_1_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (UART_1_EP_MANAGEMENT_DMA_AUTO) */

#if (UART_1_MIDI_OUT_BUFF_SIZE > 0)
    UART_1_EnableOutEP(UART_1_midi_out_ep);
#endif /* (UART_1_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF)
    UART_1_MIDI_InitInterface();
#endif /* (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF) */
}
...
#if (UART_1_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: UART_1_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref UART_1_midiOutBuffer: Used as temporary buffer between USB
    ...
    *******************************************************************************/
    ...
    {
    #if (UART_1_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (UART_1_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (UART_1_OUT_BUFFER_FULL == UART_1_GetEPState(UART_1_midi_out_ep))
        {
        #if (UART_1_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = UART_1_GetEPCount(UART_1_midi_out_ep);
        #else
            outLength = (uint8)UART_1_GetEPCount(UART_1_midi_out_ep);
        #endif /* (UART_1_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!UART_1_EP_MANAGEMENT_DMA_AUTO)
            #if (UART_1_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = UART_1_ReadOutEP(UART_1_midi_out_ep,
                                                       UART_1_midiOutBuffer, outLength);
            #else
                outLength = (uint8)UART_1_ReadOutEP(UART_1_midi_out_ep,
                                                              UART_1_midiOutBuffer, (uint16) outLength);
            #endif /* (UART_1_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (UART_1_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (UART_1_OUT_BUFFER_FULL == UART_1_GetEPState(UART_1_midi_out_ep))
                {
                }
            ...
            if (outLength >= UART_1_EVENT_LENGTH)
            {
                ...
                #if (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF)
                    if ((UART_1_midiOutBuffer[outPointer] & UART_1_CABLE_MASK) ==
                         UART_1_MIDI_CABLE_00)
                    {
                        UART_1_MIDI1_ProcessUsbOut(&UART_1_midiOutBuffer[outPointer]);
                    }
                    else if ((UART_1_midiOutBuffer[outPointer] & UART_1_CABLE_MASK) ==
                             UART_1_MIDI_CABLE_01)
                    {
                    #if (UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF)
                         UART_1_MIDI2_ProcessUsbOut(&UART_1_midiOutBuffer[outPointer]);
                    #endif /*  UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef UART_1_MIDI_OUT_EP_SERVICE_CALLBACK
                            UART_1_MIDI_OUT_EP_Service_Callback();
                        #endif /* UART_1_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    UART_1_callbackLocalMidiEvent(UART_1_midiOutBuffer[outPointer] & UART_1_CABLE_MASK,
                                                            &UART_1_midiOutBuffer[outPointer + UART_1_EVENT_BYTE1]);
                    outPointer += UART_1_EVENT_LENGTH;
                }
            }
...
#if (UART_1_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: UART_1_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  UART_1_MIDI_IN_Service() and from the
    *  UART_1_PutUsbMidiIn() function.
    *
    * \globalvars
    *  UART_1_midi_in_ep: Used as an IN endpoint number.
    *  UART_1_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   UART_1_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void UART_1_MIDI_IN_EP_Service(void) 
    {
        ...
        if (UART_1_midiInPointer != 0u)
        {
            if(UART_1_GetEPState(UART_1_midi_in_ep) == UART_1_EVENT_PENDING)
            {
            #if (UART_1_EP_MANAGEMENT_DMA_AUTO)
                UART_1_LoadInEP(UART_1_midi_in_ep, NULL, (uint16)UART_1_midiInPointer);
            #else
                UART_1_LoadInEP(UART_1_midi_in_ep, UART_1_midiInBuffer,
                                                              (uint16) UART_1_midiInPointer);
            #endif /* (UART_1_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void UART_1_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == UART_1_midiInPointer)
        {
            do
            {
                if (UART_1_midiInPointer <= (UART_1_MIDI_IN_BUFF_SIZE - UART_1_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = UART_1_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        UART_1_PrepareInBuffer(m1, (uint8 *)&UART_1_MIDI1_Event.msgBuff[0],
                                                                       UART_1_MIDI1_Event.size, UART_1_MIDI_CABLE_00);
                    }
                }
                ...
                if (UART_1_midiInPointer <= (UART_1_MIDI_IN_BUFF_SIZE - UART_1_EVENT_LENGTH))
                {
                    ...
                    m2 = UART_1_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        UART_1_PrepareInBuffer(m2, (uint8 *)&UART_1_MIDI2_Event.msgBuff[0],
                                                                       UART_1_MIDI2_Event.size, UART_1_MIDI_CABLE_01);
                    }
                }
            #endif /*  UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF */
            }
            while((UART_1_midiInPointer <= (UART_1_MIDI_IN_BUFF_SIZE - UART_1_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        UART_1_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: UART_1_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The UART_1_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  UART_1_TRUE  | Host is not ready to receive this message
    *  UART_1_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref UART_1_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref UART_1_midiInPointer: Checked this variable to see if
    ...
    ***************************************************************************/
                                                                ...
                                                                
    {
        ...
        if (UART_1_midiInPointer >
                    (UART_1_EP[UART_1_midi_in_ep].bufferSize - UART_1_EVENT_LENGTH))
        {
            UART_1_MIDI_IN_EP_Service();
        }
        ...
        if (UART_1_midiInPointer <=
                    (UART_1_EP[UART_1_midi_in_ep].bufferSize - UART_1_EVENT_LENGTH))
        {
            if((ic < UART_1_EVENT_LENGTH) || (ic >= UART_1_MIDI_STATUS_MASK))
            {
                UART_1_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else
            {
                ...
                do
                {
                    UART_1_PrepareInBuffer(UART_1_MIDI_SYSEX, &midiMsg[msgIndex],
                                                     UART_1_EVENT_BYTE3, cable);

                    ic -= UART_1_EVENT_BYTE3;
                    msgIndex += UART_1_EVENT_BYTE3;
                        ...
                        (UART_1_EP[UART_1_midi_in_ep].bufferSize - UART_1_EVENT_LENGTH))
                    {
                           ...
                           (UART_1_EP[UART_1_midi_in_ep].bufferSize - UART_1_EVENT_LENGTH))
                        {
                            ...
                            retError = UART_1_TRUE;
                            break;
                        }
                    }
                }
                while (ic > UART_1_EVENT_BYTE3);

                if (retError == UART_1_FALSE)
                {
                    UART_1_PrepareInBuffer(UART_1_MIDI_EOSEX, midiMsg, ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: UART_1_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  UART_1_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    UART_1_PutUsbMidiIn() function messages.
    *  UART_1_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        if (ic >= (UART_1_MIDI_STATUS_MASK | UART_1_MIDI_SINGLE_BYTE_MASK))
        {
            UART_1_midiInBuffer[UART_1_midiInPointer] = UART_1_SINGLE_BYTE | cable;
            UART_1_midiInPointer++;
            UART_1_midiInBuffer[UART_1_midiInPointer] = ic;
            UART_1_midiInPointer++;
            UART_1_midiInBuffer[UART_1_midiInPointer] = 0u;
            UART_1_midiInPointer++;
            UART_1_midiInBuffer[UART_1_midiInPointer] = 0u;
            UART_1_midiInPointer++;
        }
        else if((ic < UART_1_EVENT_LENGTH) || (ic == UART_1_MIDI_SYSEX))
        {
            if(ic == UART_1_MIDI_SYSEX)
            {
                UART_1_midiInBuffer[UART_1_midiInPointer] = UART_1_SYSEX | cable;
                UART_1_midiInPointer++;
            }
            else if (srcBuffZero < UART_1_MIDI_SYSEX)
            {
                UART_1_midiInBuffer[UART_1_midiInPointer] = (srcBuffZero >> 4u) | cable;
                UART_1_midiInPointer++;
            }
            else if (srcBuffZero == UART_1_MIDI_TUNEREQ)
            {
                UART_1_midiInBuffer[UART_1_midiInPointer] = UART_1_1BYTE_COMMON | cable;
                UART_1_midiInPointer++;
            }
            else if ((srcBuffZero == UART_1_MIDI_QFM) || (srcBuffZero == UART_1_MIDI_SONGSEL))
            {
                UART_1_midiInBuffer[UART_1_midiInPointer] = UART_1_2BYTE_COMMON | cable;
                UART_1_midiInPointer++;
            }
            else if (srcBuffZero == UART_1_MIDI_SPP)
            {
                UART_1_midiInBuffer[UART_1_midiInPointer] = UART_1_3BYTE_COMMON | cable;
                UART_1_midiInPointer++;
            }
            else
            {
            ...
            UART_1_midiInBuffer[UART_1_midiInPointer] = srcBuffZero;
            UART_1_midiInPointer++;
            UART_1_midiInBuffer[UART_1_midiInPointer] = srcBuffOne;
            UART_1_midiInPointer++;
            UART_1_midiInBuffer[UART_1_midiInPointer] = srcBuff[2u];
            UART_1_midiInPointer++;
        }
        else if (ic == UART_1_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    UART_1_midiInBuffer[UART_1_midiInPointer] =
                                                                        UART_1_SYSEX_ENDS_WITH1 | cable;
                    UART_1_midiInPointer++;
                    UART_1_midiInBuffer[UART_1_midiInPointer] = UART_1_MIDI_EOSEX;
                    UART_1_midiInPointer++;
                    UART_1_midiInBuffer[UART_1_midiInPointer] = 0u;
                    UART_1_midiInPointer++;
                    UART_1_midiInBuffer[UART_1_midiInPointer] = 0u;
...
*/
#if (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: UART_1_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   UART_1_MIDI_Event: initialized to zero.
    *   UART_1_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void UART_1_MIDI_InitInterface(void) 
    {
        UART_1_MIDI1_Event.length  = 0u;
        UART_1_MIDI1_Event.count   = 0u;
        UART_1_MIDI1_Event.size    = 0u;
        UART_1_MIDI1_Event.runstat = 0u;
        UART_1_MIDI1_TxRunStat     = 0u;
        UART_1_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, UART_1_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, UART_1_CUSTOM_UART_RX_PRIOR_NUM);

    #if (UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF)
        UART_1_MIDI2_Event.length  = 0u;
        UART_1_MIDI2_Event.count   = 0u;
        UART_1_MIDI2_Event.size    = 0u;
        UART_1_MIDI2_Event.runstat = 0u;
        UART_1_MIDI2_TxRunStat     = 0u;
        UART_1_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, UART_1_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, UART_1_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF) */
    ...
    #ifdef UART_1_MIDI_INIT_CALLBACK
        UART_1_MIDI_Init_Callback();
    #endif /* (UART_1_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: UART_1_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & UART_1_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & UART_1_MIDI_STATUS_MASK) == UART_1_MIDI_STATUS_MASK)
            {
                if ((mData & UART_1_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case UART_1_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = UART_1_MIDI_SYSEX;
                            rxStat->runstat = UART_1_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case UART_1_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = UART_1_MIDI_EOSEX;
                            break;
                        case UART_1_MIDI_SPP:
                            rxStat->msgBuff[0u] = UART_1_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case UART_1_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = UART_1_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case UART_1_MIDI_QFM:
                            rxStat->msgBuff[0u] = UART_1_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case UART_1_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = UART_1_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & UART_1_MIDI_STATUS_MASK)
                {
                    case UART_1_MIDI_NOTE_OFF:
                    case UART_1_MIDI_NOTE_ON:
                    case UART_1_MIDI_POLY_KEY_PRESSURE:
                    case UART_1_MIDI_CONTROL_CHANGE:
                    case UART_1_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case UART_1_MIDI_PROGRAM_CHANGE:
                    case UART_1_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == UART_1_MIDI_SYSEX)
            {
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & UART_1_MIDI_STATUS_MASK)
                {
                    case UART_1_MIDI_NOTE_OFF:
                    case UART_1_MIDI_NOTE_ON:
                    case UART_1_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: UART_1_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  UART_1_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = UART_1_ProcessMidiIn(rxData,
                                                    (UART_1_MIDI_RX_STATUS *)&UART_1_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: UART_1_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  UART_1_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  UART_1_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    UART_1_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    UART_1_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    ...
    *******************************************************************************/
    ...
    #ifdef UART_1_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        UART_1_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (UART_1_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[UART_1_EVENT_BYTE0] & UART_1_CIN_MASK;

        if ((cmd != UART_1_RESERVED0) && (cmd != UART_1_RESERVED1))
        {
            len = UART_1_MIDI_SIZE[cmd];
            i = UART_1_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == UART_1_SYSEX)
            {
                if ((epBuf[UART_1_EVENT_BYTE1] == UART_1_MIDI_SYSEX) &&
                    (epBuf[UART_1_EVENT_BYTE2] == UART_1_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    UART_1_MIDI1_InqFlags |= UART_1_INQ_SYSEX_FLAG;
                }
                else
                {
                    UART_1_MIDI1_InqFlags &= (uint8)~UART_1_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == UART_1_SYSEX_ENDS_WITH1)
            {
                UART_1_MIDI1_InqFlags &= (uint8)~UART_1_INQ_SYSEX_FLAG;
            }
            else if (cmd == UART_1_SYSEX_ENDS_WITH2)
            {
                UART_1_MIDI1_InqFlags &= (uint8)~UART_1_INQ_SYSEX_FLAG;
            }
            else if (cmd == UART_1_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((UART_1_MIDI1_InqFlags & UART_1_INQ_SYSEX_FLAG) != 0u)
                {
                    UART_1_MIDI1_InqFlags &= (uint8)~UART_1_INQ_SYSEX_FLAG;
                    if ((epBuf[UART_1_EVENT_BYTE1] == UART_1_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[UART_1_EVENT_BYTE2] == UART_1_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        UART_1_MIDI1_InqFlags |= UART_1_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= UART_1_NOTE_OFF) && (cmd <= UART_1_PITCH_BEND_CHANGE))
            {
                if (UART_1_MIDI1_TxRunStat == epBuf[UART_1_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    UART_1_MIDI1_TxRunStat = epBuf[UART_1_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef UART_1_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        UART_1_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (UART_1_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: UART_1_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  UART_1_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = UART_1_ProcessMidiIn(rxData,
                                                    (UART_1_MIDI_RX_STATUS *)&UART_1_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: UART_1_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  UART_1_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  UART_1_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  UART_1_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  UART_1_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
    ...
    #ifdef UART_1_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        UART_1_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (UART_1_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != UART_1_RESERVED0) && (cmd != UART_1_RESERVED1))
        {
            ...
            if(cmd == UART_1_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    UART_1_MIDI2_InqFlags |= UART_1_INQ_SYSEX_FLAG;
                }
                else
                {
                    UART_1_MIDI2_InqFlags &= (uint8)~UART_1_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == UART_1_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((UART_1_MIDI2_InqFlags & UART_1_INQ_SYSEX_FLAG) != 0u)
                {
                    UART_1_MIDI2_InqFlags &= (uint8)~UART_1_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        UART_1_MIDI2_InqFlags |= UART_1_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= UART_1_NOTE_OFF) && ( cmd <= UART_1_PITCH_BEND_CHANGE))
            {
                if (UART_1_MIDI2_TxRunStat == epBuf[UART_1_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    UART_1_MIDI2_TxRunStat = epBuf[UART_1_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef UART_1_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        UART_1_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (UART_1_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (UART_1_MIDI_EXT_MODE >= UART_1_TWO_EXT_INTRF) */
#endif /* (UART_1_MIDI_EXT_MODE >= UART_1_ONE_EXT_INTRF) */
...
#endif  /* defined(UART_1_ENABLE_MIDI_STREAMING) */
