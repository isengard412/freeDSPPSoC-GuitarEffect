...
I2C_S_BACKUP_STRUCT I2C_S_backup =
{
    I2C_S_DISABLE,
    ...
    I2C_S_DEFAULT_CFG,
    I2C_S_DEFAULT_ADDR,
    LO8(I2C_S_DEFAULT_DIVIDE_FACTOR),
    HI8(I2C_S_DEFAULT_DIVIDE_FACTOR),
#else  /* (I2C_S_UDB_IMPLEMENTED) */
    I2C_S_DEFAULT_CFG,
#endif /* (I2C_S_FF_IMPLEMENTED) */

#if (I2C_S_TIMEOUT_ENABLED)
    I2C_S_DEFAULT_TMOUT_PERIOD,
    I2C_S_DEFAULT_TMOUT_INTR_MASK,
#endif /* (I2C_S_TIMEOUT_ENABLED) */
};

#if ((I2C_S_FF_IMPLEMENTED) && (I2C_S_WAKEUP_ENABLED))
    volatile uint8 I2C_S_wakeupSource;
#endif /* ((I2C_S_FF_IMPLEMENTED) && (I2C_S_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2C_S_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2C_S_backup - The global variable used to save the component
...
{
#if (I2C_S_FF_IMPLEMENTED)
    #if (I2C_S_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (I2C_S_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    I2C_S_backup.cfg     = I2C_S_CFG_REG;
    I2C_S_backup.xcfg    = I2C_S_XCFG_REG;
...
#if (I2C_S_WAKEUP_ENABLED)
    /* Disable master */
    I2C_S_CFG_REG &= (uint8) ~I2C_S_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    I2C_S_PWRSYS_CR1_REG |= I2C_S_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    I2C_S_XCFG_REG |= I2C_S_XCFG_FORCE_NACK;
    while (0u == (I2C_S_XCFG_REG & I2C_S_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    I2C_S_DisableInt();
    (void) CyIntSetVector(I2C_S_ISR_NUMBER, &I2C_S_WAKEUP_ISR);
    I2C_S_wakeupSource = 0u;
    I2C_S_EnableInt();
#endif /* (I2C_S_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    I2C_S_backup.control = (I2C_S_CFG_REG & I2C_S_CTRL_ANY_ADDRESS_MASK);
#endif /* (I2C_S_FF_IMPLEMENTED) */

#if (I2C_S_TIMEOUT_ENABLED)
    I2C_S_TimeoutSaveConfig();
#endif /* (I2C_S_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_S_Sleep
********************************************************************************
*
* Summary:
...
void I2C_S_Sleep(void) 
{
    ...
    I2C_S_backup.enableState = I2C_S_DISABLE;
...
#else
    /* Store enable state */
    I2C_S_backup.enableState = (uint8) I2C_S_IS_ENABLED;

    if (0u != I2C_S_backup.enableState)
    {
        I2C_S_Stop();
    }
#endif /* (I2C_S_WAKEUP_ENABLED) */
...
/*******************************************************************************
* Function Name: I2C_S_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before I2C_S_SaveConfig() or
*  I2C_S_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void I2C_S_RestoreConfig(void) 
{
    ...
    if (I2C_S_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        I2C_S_PWRSYS_CR1_REG &= (uint8) ~I2C_S_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        I2C_S_CFG_REG = I2C_S_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        I2C_S_backup.enableState = I2C_S_ENABLE;
    #endif /* (I2C_S_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        I2C_S_XCFG_REG    = I2C_S_backup.xcfg;
        I2C_S_CFG_REG     = I2C_S_backup.cfg;
...
#else
    I2C_S_CFG_REG = I2C_S_backup.control;
#endif /* (I2C_S_FF_IMPLEMENTED) */

#if (I2C_S_TIMEOUT_ENABLED)
    I2C_S_TimeoutRestoreConfig();
#endif /* (I2C_S_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_S_Wakeup
********************************************************************************
*
* Summary:
