...
CY_ISR(I2C_M_ISR)
{
   ...
   uint8  tmp8;
...
#ifdef I2C_M_ISR_ENTRY_CALLBACK
    I2C_M_ISR_EntryCallback();
#endif /* I2C_M_ISR_ENTRY_CALLBACK */
    

#if(I2C_M_TIMEOUT_FF_ENABLED)
    if(0u != I2C_M_TimeoutGetStatus())
    {
        I2C_M_TimeoutReset();
        I2C_M_state = I2C_M_SM_EXIT_IDLE;
        /* I2C_M_CSR_REG should be cleared after reset */
    }
#endif /* (I2C_M_TIMEOUT_FF_ENABLED) */


    tmpCsr = I2C_M_CSR_REG;      /* Make copy as interrupts clear */

#if(I2C_M_MODE_MULTI_MASTER_SLAVE_ENABLED)
    if(I2C_M_CHECK_START_GEN(I2C_M_MCSR_REG))
    {
        I2C_M_CLEAR_START_GEN;

        /* Set transfer complete and error flags */
        I2C_M_mstrStatus |= (I2C_M_MSTAT_ERR_XFER |
                                        I2C_M_GET_MSTAT_CMPLT);

        /* Slave was addressed */
        I2C_M_state = I2C_M_SM_SLAVE;
    }
#endif /* (I2C_M_MODE_MULTI_MASTER_SLAVE_ENABLED) */


#if(I2C_M_MODE_MULTI_MASTER_ENABLED)
    if(I2C_M_CHECK_LOST_ARB(tmpCsr))
    {
        /* Set errors */
        I2C_M_mstrStatus |= (I2C_M_MSTAT_ERR_XFER     |
                                        I2C_M_MSTAT_ERR_ARB_LOST |
                                        I2C_M_GET_MSTAT_CMPLT);

        I2C_M_DISABLE_INT_ON_STOP; /* Interrupt on Stop is enabled by write */

        #if(I2C_M_MODE_MULTI_MASTER_SLAVE_ENABLED)
            if(I2C_M_CHECK_ADDRESS_STS(tmpCsr))
            {
                /* Slave was addressed */
                I2C_M_state = I2C_M_SM_SLAVE;
            }
...
#endif /* (I2C_M_MODE_MULTI_MASTER_ENABLED) */

    /* Check for master operation mode */
    if(I2C_M_CHECK_SM_MASTER)
    {
        ...
        if(I2C_M_CHECK_BYTE_COMPLETE(tmpCsr))
        {
            switch (I2C_M_state)
            {
            case I2C_M_SM_MSTR_WR_ADDR:  /* After address is sent, write data */
            case I2C_M_SM_MSTR_RD_ADDR:  /* After address is sent, read data */

                tmpCsr &= ((uint8) ~I2C_M_CSR_STOP_STATUS); /* Clear Stop bit history on address phase */

                if(I2C_M_CHECK_ADDR_ACK(tmpCsr))
                {
                    ...
                    {
                        I2C_M_READY_TO_READ; /* Release bus to read data */

                        I2C_M_state  = I2C_M_SM_MSTR_RD_DATA;
                    }
                }
                /* Address is NACKed */
                else if(I2C_M_CHECK_ADDR_NAK(tmpCsr))
                {
            ...
            case I2C_M_SM_MSTR_WR_DATA:
                ...
                if(I2C_M_CHECK_DATA_ACK(tmpCsr))
                {
                    ...
                    {
                        I2C_M_DATA_REG =
                                                 I2C_M_mstrWrBufPtr[I2C_M_mstrWrBufIndex];
                        I2C_M_TRANSMIT_DATA;
                        I2C_M_mstrWrBufIndex++;
                    }
                    /* End of buffer: complete writing */
                    else if(I2C_M_CHECK_NO_STOP(I2C_M_mstrControl))
                    {
        ...
        {
            I2C_M_mstrStatus |= I2C_M_GET_MSTAT_CMPLT;
    ...
    else if(I2C_M_CHECK_SM_SLAVE)
    {
    #if(I2C_M_MODE_SLAVE_ENABLED)

        if((I2C_M_CHECK_STOP_STS(tmpCsr)) || /* Stop || Restart */
           (I2C_M_CHECK_BYTE_COMPLETE(tmpCsr) && I2C_M_CHECK_ADDRESS_STS(tmpCsr)))
        {
            ...
            if(I2C_M_SM_SL_WR_DATA == I2C_M_state)
            {
                I2C_M_DISABLE_INT_ON_STOP;

                I2C_M_slStatus &= ((uint8) ~I2C_M_SSTAT_WR_BUSY);
                I2C_M_slStatus |= ((uint8)  I2C_M_SSTAT_WR_CMPLT);
        ...
        if(I2C_M_CHECK_BYTE_COMPLETE(tmpCsr))
        {
            ...
            /* Check for software address detection */
            #if(I2C_M_SW_ADRR_DECODE)
                tmp8 = I2C_M_GET_SLAVE_ADDR(I2C_M_DATA_REG);
                ...
                if(tmp8 == I2C_M_slAddress)   /* Check for address match */
                {
                    if(0u != (I2C_M_DATA_REG & I2C_M_READ_FLAG))
                    {
                    ...
                    #ifdef I2C_M_SW_PREPARE_READ_BUF_CALLBACK
                        I2C_M_SwPrepareReadBuf_Callback();
                    #endif /* I2C_M_SW_PREPARE_READ_BUF_CALLBACK */
                        ...
                        {
                            I2C_M_DATA_REG = I2C_M_OVERFLOW_RETURN;
                            I2C_M_ACK_AND_TRANSMIT;
                        ...
                        I2C_M_state = I2C_M_SM_SL_RD_DATA;
                    }
                    else  /* Write transaction: receive 1st byte */
                    {
                        I2C_M_ACK_AND_RECEIVE;
                        I2C_M_state = I2C_M_SM_SL_WR_DATA;
                ...
                #ifdef I2C_M_SW_ADDR_COMPARE_ENTRY_CALLBACK
                    I2C_M_SwAddrCompare_EntryCallback();
                #endif /* I2C_M_SW_ADDR_COMPARE_ENTRY_CALLBACK */
                    
                    I2C_M_NAK_AND_RECEIVE;   /* NACK address */
                ...
                #ifdef I2C_M_SW_ADDR_COMPARE_EXIT_CALLBACK
                    I2C_M_SwAddrCompare_ExitCallback();
                #endif /* I2C_M_SW_ADDR_COMPARE_EXIT_CALLBACK */
                }

            #else /* (I2C_M_HW_ADRR_DECODE) */
                ...
                #ifdef I2C_M_HW_PREPARE_READ_BUF_CALLBACK
                    I2C_M_HwPrepareReadBuf_Callback();
                #endif /* I2C_M_HW_PREPARE_READ_BUF_CALLBACK */
            ...
            #endif /* (I2C_M_SW_ADRR_DECODE) */
            }
            /* Data states */
            /* Data master writes into slave */
            else if(I2C_M_state == I2C_M_SM_SL_WR_DATA)
            {
                if(I2C_M_slWrBufIndex < I2C_M_slWrBufSize)
                {
                    tmp8 = I2C_M_DATA_REG;
                    I2C_M_ACK_AND_RECEIVE;
                    I2C_M_slWrBufPtr[I2C_M_slWrBufIndex] = tmp8;
                    I2C_M_slWrBufIndex++;
                }
                else  /* of array: complete write, send NACK */
                {
                    I2C_M_NAK_AND_RECEIVE;
            ...
            else if(I2C_M_state == I2C_M_SM_SL_RD_DATA)
            {
                ...
                else  /* Last byte was NACKed: read complete */
                {
                    ...
                    I2C_M_DATA_REG = I2C_M_OVERFLOW_RETURN;
                    I2C_M_NAK_AND_TRANSMIT;

                    I2C_M_slStatus &= ((uint8) ~I2C_M_SSTAT_RD_BUSY);
                    I2C_M_slStatus |= ((uint8)  I2C_M_SSTAT_RD_CMPLT);
...
#ifdef I2C_M_ISR_EXIT_CALLBACK
    I2C_M_ISR_ExitCallback();
#endif /* I2C_M_ISR_EXIT_CALLBACK */    
}


#if ((I2C_M_FF_IMPLEMENTED) && (I2C_M_WAKEUP_ENABLED))
    /*******************************************************************************
    * Function Name: I2C_M_WAKEUP_ISR
    ********************************************************************************
    *
    * Summary:
    ...
    *
    *******************************************************************************/
    CY_ISR(I2C_M_WAKEUP_ISR)
    {
    #ifdef I2C_M_WAKEUP_ISR_ENTRY_CALLBACK
        I2C_M_WAKEUP_ISR_EntryCallback();
    #endif /* I2C_M_WAKEUP_ISR_ENTRY_CALLBACK */
         
        /* Set flag to notify that matched address is received */
        I2C_M_wakeupSource = 1u;

        /* SCL is stretched until the I2C_Wake() is called */

    #ifdef I2C_M_WAKEUP_ISR_EXIT_CALLBACK
        I2C_M_WAKEUP_ISR_ExitCallback();
    #endif /* I2C_M_WAKEUP_ISR_EXIT_CALLBACK */
    }
