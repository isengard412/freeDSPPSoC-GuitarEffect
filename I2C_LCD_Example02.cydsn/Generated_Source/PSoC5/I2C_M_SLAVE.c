...
volatile uint8 I2C_M_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * I2C_M_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   I2C_M_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   I2C_M_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * I2C_M_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   I2C_M_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   I2C_M_slWrBufIndex; /* Slave Receive buffer Index */

#if (I2C_M_SW_ADRR_DECODE)
    volatile uint8 I2C_M_slAddress;  /* Software address variable */
...
*
* Global variables:
*  I2C_M_slStatus - The global variable used to store a current
...
*  The I2C_M_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 I2C_M_SlaveClearReadStatus(void) 
{
    ...
    I2C_M_slStatus &= (uint8) ~I2C_M_SSTAT_RD_CLEAR;
...
*  The I2C_M_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 I2C_M_SlaveClearWriteStatus(void) 
{
    ...
    I2C_M_slStatus &= (uint8) ~I2C_M_SSTAT_WR_CLEAR;
...
*
* Global variables:
*  I2C_M_Address  - The global variable used to store an I2C slave
...
{
#if (I2C_M_SW_ADRR_DECODE)
    I2C_M_slAddress = (address & I2C_M_SLAVE_ADDR_MASK);
#else
    I2C_M_ADDR_REG  = (address & I2C_M_SLAVE_ADDR_MASK);
#endif /* (I2C_M_SW_ADRR_DECODE) */
}
...
*
* Global variables:
*  I2C_M_slRdBufPtr   - The global variable used to store a pointer
*                                  to the slave read buffer.
*  I2C_M_slRdBufSize  - The global variable used to store a slave
*                                  read buffer size.
*  I2C_M_slRdBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        I2C_M_slRdBufPtr   = (volatile uint8 *) rdBuf;
...
*
* Global variables:
*  I2C_M_slWrBufPtr   - The global variable used to store a pointer
*                                  to the slave write buffer.
*  I2C_M_slWrBufSize  - The global variable used to store a slave
*                                  write buffer size.
*  I2C_M_slWrBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != wrBuf)
    {
        I2C_M_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        I2C_M_slWrBufPtr   = (volatile uint8 *) wrBuf;
