...
uint8 I2C_M_initVar = 0u; /* Defines if component was initialized */

volatile uint8 I2C_M_state;  /* Current state of I2C FSM */
...
void I2C_M_Init(void) 
{
    ...
    I2C_M_ADDR_REG = I2C_M_DEFAULT_ADDR;
    ...
    I2C_M_CFG_REG      = I2C_M_DEFAULT_CFG;
    I2C_M_INT_MASK_REG = I2C_M_DEFAULT_INT_MASK;

    /* Enable interrupt generation in status */
    intState = CyEnterCriticalSection();
    I2C_M_INT_ENABLE_REG |= I2C_M_INTR_ENABLE;
    CyExitCriticalSection(intState);

    /* Configure bit counter */
    #if (I2C_M_MODE_SLAVE_ENABLED)
        I2C_M_PERIOD_REG = I2C_M_DEFAULT_PERIOD;
    #endif  /* (I2C_M_MODE_SLAVE_ENABLED) */

    /* Configure clock generator */
    #if (I2C_M_MODE_MASTER_ENABLED)
        I2C_M_MCLK_PRD_REG = I2C_M_DEFAULT_MCLK_PRD;
        I2C_M_MCLK_CMP_REG = I2C_M_DEFAULT_MCLK_CMP;
    ...
    #if (I2C_M_INTERN_I2C_INTR_HANDLER)
        (void) CyIntSetVector(I2C_M_ISR_NUMBER, &I2C_M_ISR);
    #endif /* (I2C_M_INTERN_I2C_INTR_HANDLER) */
...
void I2C_M_Enable(void) 
{
    ...
    CyExitCriticalSection(intState);
        ...
        I2C_M_COUNTER_AUX_CTL_REG |= I2C_M_CNT7_ENABLE;
        CyExitCriticalSection(intState);
    #endif /* (I2C_M_MODE_SLAVE_ENABLED) */

    /* Enable slave or master bits */
    I2C_M_CFG_REG |= I2C_M_ENABLE_MS;
#endif /* (I2C_M_FF_IMPLEMENTED) */
...
*
* Global variables:
*  I2C_M_initVar - This variable is used to check the initial
...
void I2C_M_Start(void) 
{
    ...
    {
        I2C_M_Init();
        I2C_M_initVar = 1u; /* Component initialized */
    }
...
void I2C_M_Stop(void) 
{
...
#if (I2C_M_TIMEOUT_ENABLED)
    I2C_M_TimeoutStop();
#endif  /* End (I2C_M_TIMEOUT_ENABLED) */
...
#if (I2C_M_FF_IMPLEMENTED)
    {
        ...
        blockResetCycles = ((uint16) ((uint16) I2C_M_CLKDIV2_REG << 8u) | I2C_M_CLKDIV1_REG) + 1u;

        /* Disable block */
        I2C_M_CFG_REG &= (uint8) ~I2C_M_CFG_EN_SLAVE;
        ...
        /* Enable block */
        I2C_M_CFG_REG |= (uint8) I2C_M_ENABLE_MS;
...
#if (I2C_M_MODE_SLAVE_ENABLED)
    {
        /* Disable bit counter */
        uint8 intState = CyEnterCriticalSection();
        I2C_M_COUNTER_AUX_CTL_REG &= (uint8) ~I2C_M_CNT7_ENABLE;
        CyExitCriticalSection(intState);
    }
#endif /* (I2C_M_MODE_SLAVE_ENABLED) */
    ...
    /* Clear busy statuses */
#if (I2C_M_MODE_SLAVE_ENABLED)
    I2C_M_slStatus &= (uint8) ~(I2C_M_SSTAT_RD_BUSY | I2C_M_SSTAT_WR_BUSY);
#endif /* (I2C_M_MODE_SLAVE_ENABLED) */
}
