...
volatile uint8 I2C_M_mstrStatus;     /* Master Status byte  */
volatile uint8 I2C_M_mstrControl;    /* Master Control byte */

/* Transmit buffer variables */
volatile uint8 * I2C_M_mstrRdBufPtr;     /* Pointer to Master Read buffer */
volatile uint8   I2C_M_mstrRdBufSize;    /* Master Read buffer size       */
volatile uint8   I2C_M_mstrRdBufIndex;   /* Master Read buffer Index      */

/* Receive buffer variables */
volatile uint8 * I2C_M_mstrWrBufPtr;     /* Pointer to Master Write buffer */
volatile uint8   I2C_M_mstrWrBufSize;    /* Master Write buffer size       */
volatile uint8   I2C_M_mstrWrBufIndex;   /* Master Write buffer Index      */
...
*
* Global variables:
*  I2C_M_mstrStatus  - The global variable used to store a current
*                                 status of the I2C Master.
*  I2C_M_state       - The global variable used to store a current
*                                 state of the software FSM.
*  I2C_M_mstrControl - The global variable used to control the master
*                                 end of a transaction with or without Stop
*                                 generation.
*  I2C_M_mstrWrBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  I2C_M_mstrWrBufIndex - The global variable used to store current
*                                    index within the master write buffer.
*  I2C_M_mstrWrBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No
      ...
      
{
    ...
    if(NULL != wrData)
    {
        ...
        else if(I2C_M_SM_MSTR_HALT == I2C_M_state)
        {
            /* Master is ready and waiting for ReStart */
            errStatus = I2C_M_MSTR_NO_ERROR;

            I2C_M_ClearPendingInt();
            I2C_M_mstrStatus &= (uint8) ~I2C_M_MSTAT_XFER_HALT;
        }
        else
        {
            /* errStatus = I2C_M_MSTR_NOT_READY was send before */
        }

        if(I2C_M_MSTR_NO_ERROR == errStatus)
        {
            ...
            I2C_M_mstrWrBufSize  = cnt;
            I2C_M_mstrWrBufPtr   = (volatile uint8 *) wrData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            I2C_M_mstrControl = mode;

            /* Clear write status history */
            I2C_M_mstrStatus &= (uint8) ~I2C_M_MSTAT_WR_CMPLT;

            /* Hardware actions: write address and generate Start or ReStart */
            I2C_M_DATA_REG = (uint8) (slaveAddress << I2C_M_SLAVE_ADDR_SHIFT);

            if(I2C_M_CHECK_RESTART(mode))
            {
...
*                                 Stop generation.
*  I2C_M_mstrRdBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  I2C_M_mstrRdBufIndex - The global variable  used to store a
*                                    current index within the master
*                                    write buffer.
*  I2C_M_mstrRdBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(NULL != rdData)
    {
        ...
        {
            /* errStatus = I2C_M_MSTR_NOT_READY was set before */
        }
        ...
        if(I2C_M_MSTR_NO_ERROR == errStatus)
        {
            ...
            I2C_M_mstrRdBufSize   = cnt;
            I2C_M_mstrRdBufPtr    = (volatile uint8 *) rdData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            I2C_M_mstrControl = mode;

            /* Clear read status history */
            I2C_M_mstrStatus &= (uint8) ~I2C_M_MSTAT_RD_CMPLT;
...
*
* Global variables:
*  I2C_M_state - The global variable used to store a current state of
*                           the software FSM.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(I2C_M_SM_IDLE == I2C_M_state)
    {
        ...
        if(I2C_M_CHECK_BUS_FREE(I2C_M_MCSR_REG))
        {
            ...
            slaveAddress = (uint8) (slaveAddress << I2C_M_SLAVE_ADDR_SHIFT);
            if(0u != R_nW)
            {
            ...
            I2C_M_DATA_REG = slaveAddress;
            I2C_M_GENERATE_START_MANUAL;
            ...
            {
                I2C_M_BUS_RELEASE_MANUAL;

                /* Master lost arbitrage: reset FSM to IDLE */
                I2C_M_state = I2C_M_SM_IDLE;
                errStatus = I2C_M_MSTR_ERR_ARB_LOST;
            }
            else
            ...
            if(I2C_M_CHECK_ADDR_NAK(I2C_M_CSR_REG))
            {
                ...
                I2C_M_state = I2C_M_SM_IDLE;
                errStatus = I2C_M_MSTR_ERR_LB_NAK;
            }
            else
            {
...
uint8 I2C_M_MasterSendStop(void) 
{
    ...
    if(I2C_M_CHECK_MASTER_MODE(I2C_M_MCSR_REG))
    {
        I2C_M_GENERATE_STOP_MANUAL;
        I2C_M_state = I2C_M_SM_IDLE;

        /* Wait until stop has been generated */
        while(I2C_M_WAIT_STOP_COMPLETE(I2C_M_CSR_REG))
        {
        }
...
uint8 I2C_M_MasterWriteByte(uint8 theByte) 
{
    ...
    if(I2C_M_CHECK_MASTER_MODE(I2C_M_MCSR_REG))
    {
        ...
        I2C_M_TRANSMIT_DATA_MANUAL; /* Set transmit mode   */
...
uint8 I2C_M_MasterReadByte(uint8 acknNak) 
{
    ...
    if(I2C_M_CHECK_MASTER_MODE(I2C_M_MCSR_REG))
    {
        ...
        */
        if(I2C_M_SM_MSTR_RD_ADDR == I2C_M_state)
        {
            I2C_M_READY_TO_READ_MANUAL;
...
uint8 I2C_M_MasterStatus(void) 
{
    ...
    I2C_M_DisableInt(); /* Lock from interrupt */
...
*
* Global variables:
*  I2C_M_mstrRdBufIndex - The global variable stores current index
...
*
* Global variables:
*  I2C_M_mstrWrBufIndex -  The global variable used to stores current
...
*
* Global variables:
*  I2C_M_mstrRdBufIndex - The global variable used to stores current
...
*
* Global variables:
*  I2C_M_mstrRdBufIndex - The global variable used to stote current
*                                    index within master read buffer.
*  I2C_M_mstrStatus - The global variable used to store a current
*                                status of the I2C Master.
*
